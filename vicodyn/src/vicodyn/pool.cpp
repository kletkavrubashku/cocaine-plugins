    ...
    -> stream_ptr_t
{
    return peers.apply([&](peers_t& peers){
        for(size_t i = 0; i < retry_count; i++) {
            ...
            try {
                COCAINE_LOG_DEBUG(logger, "processing invocation via {}", peer->uuid());
...
auto pool_t::on_peer_state(std::future<peer_t::state_result_t> result) -> void {
...
auto pool_t::register_real(std::string uuid, std::vector<asio::ip::tcp::endpoint> ep, bool local) -> void {
    ...
    auto peer = std::make_shared<peer_t>(context, io_loop, std::move(ep), uuid, local);
